...
describe('Application Flavors', () => {
    it('allowedResults is not an empty list', () => {
        assert(Array.isArray(allowedResults))
        assert(allowedResults.length > 0)
    });

    it('allowedMonitors is not an empty object', () => {
        assert(allowedMonitors instanceof Object)
        assert(Object.keys(allowedMonitors).length > 0)
    });

    it('Every element in ALL_INPUT_TEMPLATES is an object with the required keys', () => {
    ...
    it('getAppTree returns results', () => {
        const tree = getAppTree("nwchem")
        assert('nwchem' in tree)
    });

    it('getAppTree raises on unknown application', () => {
        chai.expect(
